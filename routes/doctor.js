...
router.get("/patients", verifyLogin, async(req, res) => {
    let allpatients = await doctorHelpers.getMyPatients(req.doctor._id);
    let blocked = await doctorHelpers.getBlocked(req.doctor._id, allpatients);
    let notBlocked = await doctorHelpers.removeBlocked(req.doctor._id, allpatients);
